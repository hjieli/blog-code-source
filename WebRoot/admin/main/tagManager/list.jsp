	...
	<div class="col-lg-12">
		<div class="panel panel-default border-raduis-none">
			...
			<div class="panel-body">
				<button type="button" onclick="add()" class="btn bg-color-2 border-raduis-none">添加标签</button>
...
<script>
	...
	function add() {   //打开添加标签的弹窗
		layui.use('layer', function() {
			layer.open({
              type: 1, 
              area: ['500px', '150px'],
             title:'添加标签',
  content: '<form id="data"><table class="layui-table" lay-even lay-skin="nob"><colgroup><col width="100"><col><col width="100"></colgroup><tr><td>标签名称：</td><td><input type="type" name="name" required  lay-verify="required" placeholder="请输入标签名称" autocomplete="off" class="layui-input"/ ></td><td><button type="button" id="submitBtn" onclick="add1()" class="layui-btn">确认添加</button></td></tr></table></from>' //这里content是一个普通的String
});
		});
	}
	
	 function add1(){   //确定添加标签按钮
      if(confirm('你确定要添加吗？')){
      ...
      if(str=="" || str==null){
       alert("分类名称不能为空！");
          ...
          $.ajax({
        type : "POST",
         url : "${pageContext.request.contextPath}/back/tag/add.do",
        data : $("#data").serialize(),
    dataType : 'json',
       cache : false,
	...
	function del(id,k) {
	  if(confirm('你确定要删除吗？')){
	    $.ajax({
			type : "POST",
			url : "${pageContext.request.contextPath}/back/tag/del.do",
			data : {id:id},
			dataType : 'json',
			cache : false,
			success : function(data) {
	...
	$.ajax({
				type : "POST",
				url : "${pageContext.request.contextPath}/back/tag/edit_list.do",
				data : {id : id},
				dataType : 'json',
				cache : false,
				success : function(data) {
			...
			layer.open({
              ...
              area: ['500px', '150px'],
             title:'编辑标签',
  content: '<form id="data1"><table class="layui-table" lay-even lay-skin="nob"><colgroup><col width="100"><col><col width="100"></colgroup><tr><td>标签名称：</td><td><input type="type" name="name" required  lay-verify="required" placeholder="请输入标签名称" autocomplete="off" class="layui-input"/ value="'+id1+'"></td><td><button type="button" id="submitBtn" onclick="edit1('+id+')" class="layui-btn">确认修改</button></td></tr></table></from>' //这里content是一个普通的String
});
		});
					} else {
						//$("#msg").html(data.result);
	 ...
	 $.ajax({
	   type : "POST",
	    url : "${pageContext.request.contextPath}/back/tag/edit.do?id="+id,
